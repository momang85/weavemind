...
def _extract_source_claims(report: str) -> list[str]:
    """提取报告中的来源声明（**真正的引用声明** + 表格来源单元格）。

    只认声明句式：`数据来源：X` / `来源：X` / `引自：X` / `出自：X`（**必须带冒号**），
    或"来源为/来源是 X"。实机教训：此前冒号可省略、还包含 `来自/根据`，于是普通
    财务叙述（"增长来源与盈利质量"、"现金来自销售回款"、"净利率下降来自毛利率回落"）
    ...
    for m in re.finditer(
        ...
        # `_claim_fragment` 与后续的"结构化词/泛化词"判定
...
def auto_repair_source_labels(report: str, mislabeled: list[str]) -> str:
    ...
    if not mis_set:
        return report
    out = report
    # 1) 正文声明替换：**只改真正的引用声明**——必须带冒号（`数据来源：X` / `来源：X`），
    #    且被标片段是短名（不含句末标点）。实机教训：此前冒号可省略、还包含
    #    `来自/根据`，于是普通财务叙述被改写——"经营活动现金主要来自销售回款"
    #    变成"主要来自：基于模型知识…"，"净利率下降来自毛利率回落"同被改写。
    for c in sorted(mis_set, key=len, reverse=True):
        if any(ch in c for ch in "。！？；!?;") or len(c) > 30:
            continue                      # 长句/句子片段不是来源名，不碰
        pat = re.compile(
            r"(数据来源|资料来源|来源|引自|出自)\s*[：:]\s*" + re.escape(c)
        )
