...
**主打场景：可复核的上市公司研究工作台。** 报告里每一个数字都要能回溯到底稿并重算，
算不出来的必须显式标成"模型知识"。溯源率随任务类型差异很大，我们**在下方披露完整区间**
（8%–85%）与各自适用的任务类型，而不是只报最好那一次。
...
### 实测成绩：溯源率区间 **8%–85%**（按任务类型）

我们不报单点最优值。同一套系统在不同任务类型上的数字溯源率差异巨大，完整披露如下：

| 任务类型 | 数字溯源率 | 说明 | 来源 |
|---|---|---|---|
| A 股结构化链路（活跃度日报） | **85%**（206/241） | 财务适配器命中、报告期数据落盘 `financials.json`，数字可直接重算 | 本仓库自述，未附独立复核记录 |
| 美股链路（SEC EDGAR） | **8%**（3/37），其中金额溯源 **0%**（0/17） | 独立实测 WBD 任务。根因是结构化财务数据当时未接到该 target 上 | [新手实测报告_20260914.md](docs/新手实测报告_20260914.md) |
| 财务任务（结构化链路未命中） | **46%**，金额 **53%**，12 处占位 | 同一目标两次复现 | [试运行报告_20260912.md](docs/试运行报告_20260912.md) |
| 通用调研任务（纯搜索、无结构化源） | **0%**（0 个数字可核），15 处数据缺失占位 | 数字全部来自模型知识 | [新手实测报告_20260914.md](docs/新手实测报告_20260914.md) |
| 通用文件/代码类任务 | 不适用 | 无数字主张，不进入该指标分母 | — |

**区间的读法**：能否拿到报告期结构化财务数据，是 85% 与 8% 的分水岭——而不是模型能力差异。
结构化链路命中时数字可回溯到底稿并重算；未命中时溯源率跌到 0%–46%。

溯源率随反思迭代逐轮爬升（6% → 23% → 55% → 85%，A 股链路同一次任务内）。原始实测记录见
[docs/新手实测报告_20260914.md](docs/新手实测报告_20260914.md) 与
[docs/市场需求度评估与演进方向_20260916.md](docs/市场需求度评估与演进方向_20260916.md)。

> **美股链路的当前状态**：根因（结构化数据未接入 target）已修复，并补齐了英文/代码主体识别、
> 单位口径（不再把美股金额一律写成"亿元"）与子句级主体绑定，但修复目前只在**合成夹具**上验证
> （金额溯源率 0.833 > 0.70 阈值，夹具显式标注"非现实公司事实"）；
> 真实 `data.sec.gov` 抓取尚未跑通（依赖境外网络）。因此上表的 8% 仍是可引用的独立实测值，
> 修复后的真实成绩待补。
