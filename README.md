...
控制台新增 **「记忆与进化」** 页面（`/memory`），把系统"看不见的积累"变成看得见的资产：

**记忆库 · 它记得什么**
- 对话记忆：最近 50 条任务目标（含时间戳），展示系统"做过什么"；
- 成功策略：最近 50 条可复用解决路径（目标关键词、步数、完整步骤与成果摘要），
  展开即可看到"它从过去任务中学到了什么"——新任务会通过向量检索自动复用这些策略。

**进化锦标赛回放**
- 每一轮进化的完整回放：胜者、稳定性、是否部署、**各变异体得分板**、
  每个测试任务的 **排名与评审分数**；
- 一键「触发新一轮进化」，3-5 分钟后刷新即可看到新回放；
- 回放数据落盘在 `evolution_history.json`（已忽略，不入库）。

这是同类多智能体项目里少见的"系统自我进化可回放"能力：用户能亲眼看到
AI 团队的策略如何一轮轮自我改进，而不是黑盒。
