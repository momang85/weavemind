    ...
    async def execute(self, instruction: str, task: dict | None = None) -> str:
        ...
        try:
            ...
            system = get_prompt("report_generator", (
                ...
                "并在「关键数据一览」的来源列标注：有 [n] 编号的数字填 [n]"
                "（对应 '## 参考来源' 清单序号），"
                "结构化数据源的数字标注数据源块给出的名称；"
                "表中没有的数字若无溯源，标注"
                "'（基于模型知识，未在本次检索中验证）'，禁止编造。"
                "来源列禁止填写正文短语/指标名/观点片段"
                "（如'指出'、'主要挑战'、'能量密度提升空间有限'均不是来源）。"
                "报告时效纪律：报告中的'数据截至日期/报告日期/更新时间'必须引用"
                "[结构化数据] 或 [结构化财务数据] 块标注的数据获取时间"
                "（retrieved_at / 数据获取时间），禁止使用模型回忆的日期"
