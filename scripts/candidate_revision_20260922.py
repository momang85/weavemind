...
FIX_52_OLD = ("- 资产端减少额（24.47 亿元）大于负债端减少额（20.90 亿元），"
              "是资产负债率下降的直接算术原因。")
FIX_52_NEW = ("- 资产负债率按两期水平复算：2023 年 177.42/697.92 = 25.42%，"
              "2024 年 156.52/673.45 = 23.24%，下降 2.18 个百分点。"
              "**绝对额的减幅大小不构成比率变化的直接原因**："
              "资产与负债同时减少时，比率取决于两者各自的降幅"
              "（本例负债降幅约 11.78%、资产约 3.51%，分母降得更快，比率才下降）；"
              "资产减额更大而负债率不变的情形同样成立，因此本节只用比率复算说明变化。")
