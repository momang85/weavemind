...
**当前批次**：**S2 的"登录态渲染检查"已做**（真任务实机跑过，逐项证据见
`docs/evidence/delivery_last_mile_20260917.md`），据此修掉 4 个真实缺陷。
S1、S3 核心、S2 其余部分此前已完成。

**本轮（提交 `46c36a8`）**
- **已修并回归**：①收尾对**被采纳的交付正文**补确定性验收（此前验收只绑报告步骤的中间
  正文，交付状态恒为"该版本没有对应它自身的验收（未知）"——真实副本上验证 `''` → `fail`+4 缺口）；
  ②`elapsed_sec` 按 UTC 解析（此前多算 8 小时，实测 28854 秒/2 分钟）；
  ③无 timestamp 的日志推送落 UTC ISO（此前落服务器本地裸钟，同列日志一半本地一半 UTC，
  页面出现 `59+00:00` 残片）；④人工改版同步进交付正文（此前接口 ok 而导出字节与改版前完全一样）；
  ⑤390px 窄屏输入框被按钮组压成 24px。
- **测试**：新增/更新 12 条回归（装配正文绑定 4、时长时区 5、改版进交付 2 等）；
  并把"收尾会落盘"的完整 `run()` 用例隔离到临时工作区（此前写共享默认工作区，
  重跑同一用例会读到上一轮的验收结论）。本机 CI 全量清单 1490 项跑过，除上述已修项外无失败。
- **实机通过项**：取消（终态 `CANCELLED`，UI 响应 0.6s 与在飞结算 900s **分开**记）、
  导出草稿头与版本 id 一致、复核改版 400/400/200 三态、无事实时底稿 404、
  日志时钟按本地时区渲染、窄屏 390px 无横向溢出。
- **未修挂账**：文档级数字-主体绑定缺口（既有）。
- **已修（本轮后续）**：
  - `identity_id()` 漂移——`bind_acceptance` 改写 `rules_fingerprint` 后重算身份与入库键
    不再相等（实测 `7faeb4d9…` vs `2033fae7…`）。现在**入库时算出的身份是权威**，另加
    `identity_drifted()` 审计信号；真实任务上验证页面/导出/清单的版本标识都回到
    `7faeb4d9…`（与版本库的键一致）。
  - 运行中刷新不再跟踪任务——恢复点记进 sessionStorage，未终态才恢复（并补 running 状态，
    否则没有"停止"按钮）；只清"明确终态"的恢复点；`clearLastTask()` 不放进 `reset()`
    （挂载流程会调它）。实测重载后 35 行日志与"停止"按钮都回来。
- **环境受限**：真任务的 planner（`qwen3.8-max`）连续两次调用失败、Embedding 端点额度耗尽
  → 回退单步、无检索结果、验收 `fail`（数字溯源 0/63、引用 0）。这证明**缺口会被如实暴露**，
  不能当作研究链路或报告质量的证据。
- **重启后已在页面复核**（`launcher.py stop/start`，16/16 存活）：运行中 `elapsed_sec`
  11→101 秒（此前同场景 28854）、日志时钟为本地时间、交付理由显示"验收未通过（overall=fail）"、
  新任务的选中版本拿到**属于它自己**的验收（验收 hash = 正文 hash）、人工改版后导出字节**含修订**。

**当前提交**：`7df3c3d`（远端 main 已同步，**CI 四作业全绿**：run `35303129993`——
backend / frontend / docker-image / clean-env-e2e 全 success）。
历史：`2b49e81`、`ae0761f` 四作业全绿；`7338713`/`c9b671d`/`c7eb111` 上
`clean-env-e2e` 间歇失败（两类模式，根因均已确认并修，见下）。

**第二次真实公司研究（端点在用，规划请求失败）**
- 端点探针三模型均 `ok`、规划形状小请求 4.7–9.2s 正常 → 不是模型名或端点不可用；
  但产品里的**真实规划请求**（max_tokens=8192 + 注入的历史教训/格式要求）两次尝试均失败
  （降级记录 12 条、末条 `both_failed=true`）。后果同前：回退单步 → 无检索 → 报告零来源
  → 验收 `fail`（0/56、引用 0），交付如实标未验收草稿。
- 该运行暴露并已修：**验收器修过的正文就是交付正文**。此前收尾送验 13302 字节、
  验收器改写为 13458 字节诚实披露版并绑定，而交付仍是修复前那份 → 状态回到"未知"。
  真实副本验证：`unknown` → `draft（验收未通过 overall=fail）`，交付正文 13302→13458。

**部署烟测的第二类间歇失败（根因未确认，已使其可诊断）**
- `c9b671d` 上：任务收执拿到了，但 `task_history` 900 秒内一直不存在（只有重复的
  "表尚未就绪"），按"未进终态"红。CI 上没留下现场。
- 已做：任务库路径**显式钉住**（`WEAVEMIND_DB=<clone>/agents.db`，服务与读取方同一文件）；
  收执后 120s 仍读不到任务行时，把库路径/文件状态 + `logs/*.log` 尾部写进日志与证据。
  **根因未确认**——这一步交付的是"下次能定位"，不是"已修好"。

**部署烟测间歇变红：两类根因都已确认并修**
- **第一类（`7338713`）"闸门由外部搜索决定颜色"**：替身计划里的 `web_search` 抓 Bing，
  CI 上间歇拿不到可解析结果 → 触发 search-fallback 的 `code_execution` 步骤 → 该步骤失败
  并替换步骤 1 的结果 → 下游 `Blocked by failed dependency` → 任务 FAILED。修法：替身计划
  不再排检索步骤 + 烟测目标改为自足；`does_not_prove` 写明"不证明真实检索/抓取链路"。
- **第二类（`c9b671d`/`c7eb111`）"任务跑完但写不进库"**：上一轮加的现场诊断在 `c7eb111` 上
  直接给出根因——`no such table: task_history`，编排器连续 3 次落库失败后"不标记为已终结"，
  任务永远停在 RUNNING、门禁 900 秒超时。库文件存在，只缺这张表；建表此前只由
  `web_ui._init_db()` 负责，而任务提交走 Redis 不经 HTTP。修法：**写入方自己保证表在**
  （`task_state` 补列前先 `CREATE TABLE IF NOT EXISTS`）。新增 3 条回归。
- 两处成功判据与负向用例**一条没改**：修的是产品/夹具的确定性，不是把闸门调松。
  `7df3c3d` 上四作业全绿。

**S2 已完成**（提交 `7782206` + `2609f65`，CI 在 `2609f65` 四作业全绿）
- 后端：`GET /api/task/<id>/working_paper`（只读底稿，无底稿 404）；
  `POST /api/task/<id>/review/edit`（修订版=新版本、同验收器重验、**旧批准不迁移**、
  终态拒绝、未命中/未变 400 不动东西）——6 项测试已接 CI。
- 前端：`buildResearchGoal`（公司+两年度+口径+截至日 → **符合契约的目标**，缺字段只报缺口不拼目标）；
  `summarizeWorkingPaper`（缺失字段显示"未知"而非 0）；`SubmitPanel` 增"研究一家公司"表单；
  新增 `WorkingPaperPanel` 接进结果页（目标达成 → 缺口/待核验 → 关键指标含 fact_id/主体/期间/
  单位/来源/核实状态 → 计算值带公式与输入 fact_id）。
- dist 指纹：`index.html` sha256 `d423a54a14aea9704c5638c520efadb2`，
  入口 `index-_CZ2JZP_.js`、`TaskConsole-BopeBRBb.js`、`ReportViewer-Dc0GEc9t.js`；
  离线页面验证：`/`、`/api/health`、入口 JS 均 200，产物里能查到新界面文案。
- CI 的字号下限守卫拦下我新组件里的两处 `text-[11px]`（12px 为可读性下限）→ 已改，
  本地重跑守卫 36 项全绿。

**当前提交**：`aef491f`（S1 收尾）· 远端 `main` 已同步。

**S1 已完成并验证**
- 三处保真断点（币种单位/单主体/clean 归属）→ 契约与最小事实记录 → 可重算底稿与退出关卡
  → 底稿与缺口接进任务产物（`working_paper.json/csv`，缺口写进交付物本体）。
- **真实数据整链验证**（`docs/evidence/real_data_chain_20260917.md`）：一次真实抓取
  （东财公开接口，贵州茅台 2020–2025 六期年报）→ 27 条事实 → 6/6 必需组合齐备、
  无缺口无问题，同比由底稿重算（营收 +15.66%、净利 +15.38%、经营现金流 +38.85%）；
  单位/主体/期间在真实数据形状下不失真。真实抓取不进 CI，CI 跑冻结快照。

**已验证（按提交分别记账）**

- **CI 四作业全绿**：`9a7b977`（S0 全部改动）、`6077ce8`（S1-1/2/3，run `35214951068`）、
  **`aef491f`**（S1 收尾 + 真实数据快照，run `35217634029`）。
  含 Linux 侧 PDF 字形修复、部署烟测的内容判据与负向用例、跨层保真、契约与事实记录、
  底稿与退出关卡——都在 Ubuntu 上跑过。
- 本机全量：`test_p0` 399、`test_orchestrator_v2` 68、`test_delivery_chain` 184、
  `test_fact_fidelity` 8、`test_facts` 12、`test_working_paper` 13、`test_financial_chain` 31 等全绿。
- PDF 逐页视觉：Windows 两轮判定（8 页 → 6 通过/2 缺陷；修复后 3/3 通过），
  证据 `docs/evidence/pdf_visual_20260917.md`。

**未验证（不要按已完成记账）**

- **真实模型报告质量未验证**：真实数据整链走通的是"抓取→事实→底稿"，全程**未调用真实模型**
  （本机 key 余额不足）；美股真实 SEC 抓取仍待验；东财适配器未声明报表口径（底稿记 unknown）。
- 底稿/缺口目前只在**任务工作区与交付物**里；导出包（ZIP/清单）与前端展示属 S2。
- 银行投研/对公试点、港股/美股真实覆盖：按新指令后置，前置门槛不降。
- Linux 侧逐页渲染仍只在 CI 单测层面确认（未在 Linux 上跑 `scripts/pdf_visual_check.py`）。

**S2 收尾（提交 `8739660`，CI 绿）**
- 导出带底稿：清单登记 `working_paper_json/csv`（各自字节 hash）+ 摘要
  （goal_met / rows / derived / gaps / problems）；新增 `GET /api/task/<id>/working_paper.csv|json`。
- 状态与显示口径：`/api/task/<id>` 补 `phase` / `elapsed_sec` / `review_state` / `delivery`
  （**拿不到就是 null，不默认"通过"、不用 0 冒充**）；前端 `formatLocalTime`（本地时区，
  非法值→"时间未知"）与 `formatBudgetLabel`（**0/缺省=不限**，不是"用尽"）统一口径。
- 前端行为测试 27 项、`tsc`、`npm run build`、字号守卫 36 项、`test_review_edit_api` 8 项全绿。

**S3 核心（提交 `fa30082`，CI 绿）**
- `trial_metrics.py` + `scripts/trial_record.py` + `docs/试用任务包_20260917.md`：
  6 项最小指标（首份可复核报告耗时 / 必需指标覆盖 / 错误认证数 / 人工修改字段数 /
  复核耗时 / 费用已知-估算-未知 / 是否愿再做第二个任务）；**未知不当 0**（未采集须写原因，
  汇总时单列、不进中位数与总额）；**匿名**（只有试用号 + 任务 id，无 username/email/ip）；
  类别认不出记 unknown。指南按"安装→研究一家公司→看结果→复算→修订重验→导出"编号并写明边界。

**剩余（都需要你参与，我不能代办）**
- **登录态渲染检查**（桌面/窄屏、运行/成功/缺口/取消、SSE 重连、切任务不串状态）：
  按纪律不索取/复制 cookie，登录由你完成；离线检查（产物含新界面 + 页面 200）不替代它。
- **一次真实试用记录**：需要模型 key 有余额（现在余额不足，真任务会在余额预检被拒）；
  `python scripts/trial_record.py add --task-id <id> --category trial …` 即可采集。
- 银行投研/对公试点（F 批前置）、港股/美股真实覆盖：按指令后置，门槛不降。
- 文档级数字-主体绑定（已在对抗测试里记录为已知缺口）。

**下一步**：等你登录后做渲染检查 + 跑一次真实试用；其后按架构指令排港股/美股真实覆盖与银行前置。
