...
**当前批次**：F2（让简报回答金融研究问题）——代码与单测完成；**两次实机验收进行中**。
证据：`docs/evidence/report_analysis_F2_20260919.md`。

| 层 | 状态 |
|---|---|
| 功能实现 | F2-1 定向取证（`narrative_evidence.py` + 研究路径两个抓取步骤）、F2-2 简报四节（业务背景/变化解释/风险与核查/附录定位）、F2-3 两种阅读视角 + 比率适用条件——均已落地 |
| 单测 | 新增 `test_narrative_evidence.py` 15 项 + 扩展 20 余项；CI 列出的 46 个后端测试文件本地逐个跑通，前端 45 项行为测试通过 |
| 真实报告 | **两次实机进行中**：投研视角 + 银行对公视角各一次（预声明上限：单次 ≤25 分钟、失败不重跑、不切模型、不改全局预算）；读数在实机完成后回填 |
...
**下一步**（架构指令顺序）：F3 图表分面、修订前端入口、离线三场景与人工评分 → D 真实试用。
