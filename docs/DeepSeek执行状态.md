...
**当前批次**：F3-A（金融语义与来源准入收口）——代码与单测完成；**有界实机因 LLM 端点
余额不足未发起**（primary/backup 均 `insufficient_balance`，任务未创建、未耗调用）。
证据：`docs/evidence/f3a_semantics_admission_20260920.md`（四项反例前后、三稿回放、
真实匹配年报定位、版本核实）；F2/F2′ 证据见 `report_analysis_F2_20260919.md`。

| 层 | 状态 |
|---|---|
| 功能实现 | A1 现金覆盖三分支 + 研究对象类型（`subject_type`，与视角分开；金融机构禁用企业口径比率与比率图）；A2 全入口来源准入单一规则（`admission_of`，正文引用不能绕过，rejected 照实说明）；A3 完整日期精度（日/月/年，不虚构月初年初）+ 主体 unknown 不当 applicable + 缓存 rules_version |
| 单测 | 新增 13 项（准入 4 / 日期与身份 4 / 现金覆盖与金融机构 3 / 契约往返 1 / `_located` 收紧随 A3 反例）；相关 9 套件 627 项通过 |
| 实机 | **共 7 次已收尾**（2 次茅台 F2 + 3 份 21:53 三场景稿 + 历史 2 次，明细见 F3-A 证据 §6）；F3-A 实机 1 份（洋河对公，≤25min/≤40 调用）**因端点余额不足未发起**——提交返回 503"全部 LLM 端点余额不足"，任务未创建、未耗调用，不切模型不绕过 |
| 版本核实 | 三份 21:53 场景稿装载的是 19:30 进程（早于 46ce8c1/22667f8）——其缺陷不计为 22667f8 回归；F3-A 验证前服务已于 00:18 重启（16/16，无活跃任务） |
| 人工验收 | **未做**（F3-B：图表分面/结果页顺序/修订入口 + 研究员五项评分 ≥8/10） |
