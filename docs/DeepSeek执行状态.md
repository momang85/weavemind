...
**当前批次**：E（研究报告的经济分析）——**代码与单测完成；真实报告效果部分验证；人工验收未做**。
证据：`docs/evidence/report_analysis_E_20260919.md`。

按四层分开记（避免"全量全绿"代替可读输出）：

| 层 | 状态 |
|---|---|
| 功能实现 | E1 派生比率、E2 报告写全、E3 财务分析图、E4 分析覆盖检查（只提示）——均已落地 |
| 单测 | 新增 18 项（比率 8 / 分析覆盖 5 / 图表 5）；CI backend 全部 45 个测试文件本地跑通 |
| 真实报告 | 实机 `ui-ecb93e57a1`：正文 4324→**6805 字**、六项必需事实 **6/6**、同比 **3/3**、比率 **6/6**；分析检查读数"数值缺 0/6、派生缺 0/11"。**但该轮跑在图表守卫补全之前**（交付里仍有 5 张检索统计图），修复后的图只在本地渲染确认过，**实机复验待下一轮** |
| 人工验收 | **未做**（架构指令里属 F3：研究员五项评分 ≥8/10，是研发验收、不自动改写 runtime overall） |
