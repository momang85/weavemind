...
| 单测 | 新增 `test_scenarios.py` 7 项 + 图表分面/顺序/图注不变量等回归；相关 6 套件 677 项通过；前端 45 项行为测试通过、`npm run build` 通过 |
| 实机 | 三场景离线核对全通过（①verified 4 证据 5 图 ②draft 记单位/基期问题不硬算同比 ③draft 超截止文章写明原因）；浏览器实机确认任务页新面板与顺序、payload 字段（findings 4/缺口 5 组/定位 9/图 3） |
| 未验证 | 修订入口的真实人工修订端到端、图表分面后的真机产出（下一轮预声明预算的实机）；`visual-judge` 本次账号不可用，图表视觉回退人工核对 |
