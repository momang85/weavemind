# DeepSeek 执行状态（2026-09-22 晚更新）

**当前批次**：阶段 D · **09-22 晚间收口 + 浏览器正常入口测试**。收口第一小批（验证条件绑定完整事实、就绪分母来自契约、失败分支闭合，`2f03488`）、主文收敛两步（图表元数据外置 `233590e`、完整模型稿另存审计文件 `c7056d2`）已提交；今晚以登录会话走正常页面入口完成六组测试（证据 `docs/evidence/browser_ui_acceptance_20260922.md`）：工作台结构、修订正向链（真实新版本 `5d0116b6`，验收 pass 绑定本版）、安全负例（未带凭据 401 / 穿越 404 / 文件白名单）、按需导出与清单刷新（PDF `5abe9e09` 字节一致）、研究状态待重验如实显示、新 PDF 15 页逐页目视。
**未追加付费实机、未换模型、未改全局 0/0/0、未改 templates/权限。**

| 层 | 状态 |
|---|---|
| 晚间收口小批 | 同主体跨指标同值不再被提升；归因/比率校验要求对应派生或原文支持（空底稿反例 fail）；契约必答分母来自契约（缺 2 项判 1/3 而非 1/1）；证据/规则指纹变化即失效；异步 415 两条调用记录；账本初始化无锁不写；PDF 空工件不二次抓取。定向测试冻结反例全绿 |
| 浏览器正常入口 | 修订 UI 正向链（页面表单 → review/edit → 新版本+重验+重装配+导出块跟随）首次以真实点击跑通；负例 find 不匹配报 `find 未出现在当前版本正文中` 不建版本；导出 MD/PDF 响应头绑当前版本、清单同 sha 刷新 |
| 新 PDF 逐页 | 15 页：无豆腐块、减号可读、6 图嵌入、图题去内部 id。遗留版面/内容 6 处：p6 孤立标题、p8 负号与数字断行、p9 §6.3 表缺失、p10 空节、p12 字面 `**`、p5 句号甩行首（后四项为模型正文既有问题） |
| 未验项（如实） | ①同版 ZIP 未打（旧包 dc60226e 仍陈旧，页面如实提示）②修订装配已重建结构投影但 research_state 写端未补 evidence/rules/contract 指纹，状态仍待重验（读端正确）③主文 2–4 页收敛未做 ④真实 Redis 多进程、公开原文获取的整链实机仍未验 ⑤本轮无新增付费实机，前置关卡未全过不新增 |
