...
**当前批次**：F3-A（金融语义与来源准入收口）——代码、单测与**一份有界实机**均完成；
实机暴露的三处问题已修（待下一轮实机复跑）。端点已按用户提供的新 key 切到
ARK（`glm-5-3-flash-260828`），凭据只写在本地 `config.json`（gitignore，已备份）。
证据：`docs/evidence/f3a_semantics_admission_20260920.md`。

| 层 | 状态 |
|---|---|
| 功能实现 | A1 现金覆盖三分支 + 研究对象类型（金融机构禁用企业口径比率与比率图）；A2 全入口来源准入单一规则；A3 完整日期精度 + 主体/时点未核实不准入；**实机后补修**：期间未标注可准入、无标题页段落回退（含付费墙守卫）、诚实披露不计占位、`[n=]` 记号归一 |
| 单测 | 新增 19 项；相关 6 套件 749 项通过（含对抗用例仍判 fail） |
| 实机 | **F3-A 一份：`ui-f00775cfdf`**（洋河对公，22m20s / 9 次调用，均在上限内）——真机确认覆盖读数「低于」、分析完整保留 5979 字符、引用清单只含准入来源、先装配再同版验收；终态 SUCCESS_WITH_ISSUES、交付 draft（验收 fail，不冒充通过）。共 8 次实机已收尾（明细见证据 §6）；费用全部**未知** |
| 端点 | 新 key 验证：`/chat/completions` + 流式 + `include_usage` 均通过；用户给的 `/responses` 形状返回 InternalServiceError（不需要，未改客户端） |
| 人工验收 | **未做**（F3-B：图表分面/结果页顺序/修订入口/导出 + 研究员五项评分 ≥8/10） |
