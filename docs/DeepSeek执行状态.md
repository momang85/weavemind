...
**当前关卡**：M0 / R0b / R0c / MKT-P0 / R1 / R2 / 检索质量统一 已落地；**B 批代码侧完成**
（只剩"逐页视觉验收"因本机无栅格化渲染器做不了）。CI 曾在 `ec4f942` 全绿；
`b280cea`/`e7697a7`（B 批修复）的 CI 结果**待代理恢复后核对**。

**B 批（PDF 排版）：代码侧完成，并从 CI 日志里挖出两个 Linux 渲染真缺陷**
