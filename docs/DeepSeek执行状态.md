...
**当前批次**：F1（分析内容保留 + 确定性装配）——代码与单测完成；真实报告效果待下一轮实机。
证据：`docs/evidence/analysis_preservation_F1_20260919.md`。

| 层 | 状态 |
|---|---|
| 功能实现 | F1-1 内容保留与脱敏诊断（`cf85ca3`）、F1-2 研究简报代码装配（`44751cf`）——均已落地 |
| 单测 | 新增 10 项（注入过滤 3 / 报告清洗 2 / 简报装配 5）；CI backend 全部 45 个测试文件本地跑通 |
| 真实报告 | **待下一轮实机**：简报与图表只经离线与本地渲染确认（离线研究路径 `acceptance=pass`、终态 SUCCESS） |
| 人工验收 | **未做**（架构指令里属 F3：研究员五项评分 ≥8/10） |

**F1 做了什么**
- **F1-1 内容保留**：反引号规则从"任意成对反引号"收窄为"看起来是命令"（`` `CNY` `` 不再被判
  命令注入，`` `rm -rf /` `` 仍拦）；注入处理改为**结构优先 + 逐行隔离**（JSON 按结构裁剪后仍是
  合法 JSON，散文只替换命中行）；报告 worker 去掉"规模/玩家/趋势"行业模板门槛，清洗隔离标记与
  截断 JSON，被隔离资料写成**缺口**；新增任务级 `content_filter_events.jsonl` 脱敏诊断
  （规则 ID/来源步骤/内容类型/前后长度/隔离状态，不记正文）并在任务页暴露汇总。
- **F1-2 确定性装配**：新增 `report_brief.py`——关键发现/财务对照（含同比与比率的可复算公式）/
  图表/引用编号/资料范围/声明由**代码**装配，模型只写"分析"一节；来源清单只收**实际采用**的来源
  并分三类标识，未采用只留内部审计；研究简报与工程交付**分模板**（工程说明移出正文、留在任务
  详情）；结构化对象落 `report_structure.json`。
- 一并修掉四个真缺陷：中性小节名被当"别家公司"、**"归母"被 `_extract_company` 当公司名**
  （导致"归母净利润"行的数字不可溯源）、增长率标准写法被溯源正则截断、简报链接重写晚于验收。

**下一步**（架构指令顺序）：F2 有证据的经营分析（业务背景/变化解释/附注取证）→ F3 图表分面、
修订前端入口、离线三场景与人工评分 → D 真实试用。
