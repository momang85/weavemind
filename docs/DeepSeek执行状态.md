...
**当前批次**：实机端到端验证 + 五处真问题修复（浏览器视觉测试驱动）。
证据：`docs/evidence/live_e2e_yanghe_20260921.md`；提交 `4aee354`（版本口径/图表渲染）、
`4dc1742`（装配幂等）、`29ec71d`（交付包陈旧信号）。

| 层 | 状态 |
|---|---|
| 实机任务 | 洋河对公 ui-31305a2b28：11:49 提交、约 20 分钟完成（上限 25 分钟内），SUCCESS_WITH_ISSUES → 修订重验后 verified；预算 0/0/0；费用不可读记"未知" |
| 修复 | ①同页两个"当前版本"（复核栏正文版号 vs 导出行谱系标识）→ 导出块双命名空间；②正文图表全部 404（整段编码把 `charts/` 编成 `%2F`）→ 逐段编码；③1038×121366 超高图页面隐形 → 同 chart_qa 阈值占位；④装配不幂等（`****`/重复/回食压缩提示与待核查头/H1 成主张，两轮 0→3→6）→ 修复并回归；⑤清单重写未重建 zip → `package_stale` 如实标注 |
| 验证 | 修订版 PDF：6/6 图嵌入在页、0 占位、表格对齐；离线三场景 risks 无伪影无重复；test_delivery_chain 246、guards 45 等全绿；面板机器/人工两行、绑定警告、包陈旧提示均 DOM 实测 + 截图 |
| 遗留 | 现采纳正文仍带旧渲染垃圾（干净重装配候选 8b23aad3 未被采纳——修订路径语义）；反思重写稿未进最终装配（编排层待决策，未擅改）；分析节 214 字按不编造呈现；评审未完成（降级） |
