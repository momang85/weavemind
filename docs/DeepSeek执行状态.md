# DeepSeek 执行状态（2026-09-26 更新 · N4 六场景实机复验 + 新人指导）

**当前批次**：N4 场景实机复验（用户点名：干净环境 1/2、断网 4、端口冲突与 Redis 5 的 5、
错误密钥 7、含代码任务提示 8，并完善新人指导）。交付物：运行包
`dist/weavemind-2026.09.26.4-win-x64.zip`（sha256
`41429db2d1a79aa6b19d5528dd433ab6e248ab185d07df8a752c422c3be7ea89`，23,983 文件，
`secrets: 0`、`dev_paths: 0`）。

- **六场景结果**：1/2 净化环境整链**通过**（包内 Python 3.11.9、依赖 14/14、Redis 来自包内、
  免 Node、16/16 服务、工作台 HTTP 200 @ 8080、退出码 0）；4 断网冷启动**通过**（清空实例状态 +
  死代理下仍复用包内 Redis 压缩包）；5 端口冲突与 Redis 5 **通过**（8080 被占→本实例 8081；
  6379 上是 Redis 5→本实例 6380；占用方进程全程存活、`stop` 不动它）；7 错误密钥**通过**
  （向导与页面均判"鉴权失败"、`/api/config` 不回显、诊断脱敏无泄漏、2 次被拒请求零费用）；
  8 代码隔离提示**三处可见**（启动报告、页面提交前、任务执行前）。
- **本批修掉 10 个真实缺陷**（都是演练暴露的，逐条列在证据文档 §2）：运行包不用包内解释器；
  包根不在 `sys.path`（`ModuleNotFoundError: db_paths`，包内验证须同时导入项目模块才算真验证）；
  回环健康检查被代理接走；默认端口被占即报错退出（改为让位并落盘）；配置未完成把新人丢在控制台；
  Windows `isatty()` 对 NUL 也报 True；内置演示未随包；引导清掉用户已有高级配置；
  含代码任务缺"执行前"提示；`text=True` 子进程输出按 UTF-8 解码在中文系统抛栈（`_is_alive`
  的 PID 判断随之失效）。
- **新人指导**：包内 `运行说明.txt`（构建器生成）+ `docs/新人上手指南.md`（该走哪条路、逐行对照
  首次启动输出、出错→下一步表、验证程度含未验项）+ 部署指南方式 A 拆成"运行包/源码"并清掉
  过时口径（自动启动 Docker Desktop、固定映射 6379）+ 中英 README 快速开始表与验证范围。
- **未验（照实）**：另装一台真正干净的 Windows；真机物理断网（当前是进程级代理屏蔽）；
  真实 Redis 5（当前是协议桩）；含 `code_execution` 的付费任务真机提示（需真实模型调用，
  本批未跑研究任务）；浏览器里渲染新提示行（需登录态，本轮以 HTTP 接口核对字段）；
  真人评分《研究员评分表_F3》仍空。
- **本批付费请求数 0**（仅 2 次被拒的鉴权请求）；未改模型/权限/`templates.json`/用户配置；
  未发布 Release、未上传任何数据。
- 证据：`docs/evidence/n4_scenario_drills_20260926.md`。

## 归档批次（N0–N4、R1–R4 与更早）
