...
**当前批次**：阶段 D · 夜间纠偏**第一、二小批与第三小批（资料侧）完成**——①结论与完整正文审查（金额变化与
...
`docs/evidence/d3_night_correction_batch3a_material_20260921.md` 及此前各批证据。
...
| 夜间纠偏第二小批（D5 补修） | ①**每次真实发送一张票**：415 流式→非流式回退在同步与异步两条路径都独立开票；预算拒发不再重试/切备用（`_is_budget_refusal`）②**账本落盘跨进程原子**：锁文件 + `msvcrt`/`flock` 覆盖整段读改写；**账本身份初始化竞态**修掉（锁内确定/采纳已有身份）——两进程各预留 40 次此前只剩 40、现在 80③**显式有界任务 fail closed**：`multiprocess=True`（生产默认）+ 配了上限 + 共享账本不可用 → 第一次付费请求即拒，原因进交付说明，产物照常保存④`snapshot()` 分列 `provider_requests` 与 `steps_dispatched`；`tokens.actual`/`unknown_calls` 分列实际用量与上界（未知不填 0） |
| 夜间纠偏第三小批（资料侧） | ①检索查询从**结构化契约**生成（`[检索查询] 主体 代码 各年度年年度报告 指标`），`SearchAgent` 以该行构造变体并排第一（不再把整段任务要求扔给检索器）②候选**抓取前**排除：错主体（集团≠上市公司）/晚于资料截至（含 `20251105` 紧凑写法）/报告期不在契约期间，判据复用 `narrative_evidence` 的同一套解析；元数据缺失不排除③PDF 接入统一契约：`web_fetch_worker` 改走 `net_policy.fetch_document`（已验 IP、不跟随重定向、字节上限、审计），按 MIME+魔数识别 PDF → `text` 置空 + `content_hash` + `pdf` 标记，不再把截断字节当正文；`_try_pdf_evidence` 认该标记继续走解析/页码通道 |
...
| 账本核对（**对不上 → 两批修复完毕，实机待复验**） | 三处缺口：①LLM 路径建账本**没接跨进程后端**（等于每进程各一份上限）②`call_llm_stream`/`call_llm_async` **无票据、无调用记录**（步骤主路径不入账）③`_save` 整份覆盖（阶段明细被最后写者抹掉）。已修：工厂提为 `root_budget.default_redis_factory` 并共用、流式/异步/备端点每次真实发送开票结算 + 补调用记录、文件改为 `writers` 逐进程增量合并（身份不同不并）；`async_worker_base` 不再把预算拒绝当流式失败吞掉。**第二小批已闭合**：415 回退独立开票、`writers` 读改写加跨进程锁（并修账本身份初始化竞态）、有界任务在共享后端不可用时 fail closed、调度票据与 provider 请求分列、上界与实际用量分列。仍未验：真实 Redis 多进程压测与修复后实机核对 |
