...
- **未修挂账**：文档级数字-主体绑定缺口（既有）；部署烟测第二类间歇失败的根因（已改为
  失败时可诊断，未确认原因）。
- **已修（本轮后续）**：
...
**当前提交**：`79fe4a0`（远端 main 已同步，**CI 四作业全绿**：run `35250964067`——
backend / frontend / docker-image / clean-env-e2e 全 success）。
历史：`2b49e81` 四作业全绿（run `35246187852`）；`7338713`/`c9b671d` 上 `clean-env-e2e`
间歇失败（两类模式，前者已修、后者已改为可诊断）。
...
**部署烟测间歇变红：根因与修法（同批）**
- 现象：`7338713` 上 backend/frontend/docker-image 通过、`clean-env-e2e` 失败；
  日志显示任务 27 秒内 FAILED（`gh run view --log-failed` 取的）。同一签名在改动前的
  `0330b6f` 上出现过 → 非本批引入。
- 根因（本机隔离复现 + 读克隆库 `steps_json`）：替身计划里的 `web_search` 抓 Bing，
  CI 上间歇拿不到可解析结果 → 触发 search-fallback 的 `code_execution` 步骤 → 该步骤失败
  并替换步骤 1 的结果 → 下游 `Blocked by failed dependency` → 任务 FAILED。
- 修法：替身计划不再排检索步骤（改 `content_summary`）+ 烟测目标改为自足；
  `does_not_prove` 里写明"不证明真实检索/抓取链路"。成功判据与负向用例一条没改。
  本地连跑 3 次全绿（改动前 3 次里 2 次红），CI 在 `2b49e81` 上恢复全绿。
