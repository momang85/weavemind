...
**当前关卡**：M0 已实现并通过**部分**定向验证；**本批 = R0b/R0c**（版本真实身份 + 统一交付谓词；
前四批已提交推送，本批在工作区**未提交**）。下一批 = **MKT-P0 余下三项**（便携 Redis
国内多源 / SEC EDGAR 接进 target + 硬绑定 / README 区间披露）。台账见
...
- **未做（本批明确留下一项）**：验收侧还没有"**数字-主体**"维度（记录只带 指标/期间/币种/单位），
  所以"报告写宁德时代 1741 亿、来源其实是比亚迪 1741 亿"这类**同值不同主体**仍可能算可溯源——
  `test_acceptance_adversarial.py` 里那条用例正等待被翻成 `assertFalse`。这是 MKT-P0-3 的剩余一半。
