...
**当前关卡**：M0 / R0b / R0c / MKT-P0 / R1 / R2 / 检索质量统一均已落地。共 9 个提交在本地
**未推送**（`c1a7559`…，本机 GitHub 凭据失效，需人工重新认证一次）。下一批 = **B–F**
（PDF 排版 → 中文进度主视图 → L03/L06 → 冻结样本 → 银行前置）。
...
- 回归：新增 `test_search_quality_unified.py`（17 项：两条路径同源、默认值不变、
  ...
  `test_p0` 388、`test_orchestrator_v2` 68、`test_delivery_chain` 184、`test_prompt_system` 39 全绿。
