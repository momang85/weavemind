...
**当前批次**：E 完成——让研究报告有"经济分析"（派生比率 / 报告写全 / 财务分析图 /
分析覆盖检查）。证据：`docs/evidence/report_analysis_E_20260919.md`。
C 亦完成（固定研究路径、口径证据链、流式传输、实机交付级核对）：
`docs/evidence/research_path_C_20260918.md`、`docs/evidence/caliber_evidence_chain_20260918.md`。

**E 批（起因：产品负责人从经济学家角度审 `ui-2084c2c9cc`，指出报告与图片"少了东西"）**
- **派生比率**（底稿）：净利率、经营现金流对净利润的覆盖、资产负债率、研发投入强度——
  由已选事实算、带公式与输入 fact_id；缺输入不生成、分母为 0 只记审计提示（不因此判不达标）。
- **报告写全**（生成侧）：新增 `_REPORT_ANALYSIS_REQUIREMENTS` 骨架（关键数据一览表 /
  逐项同比解读 / 盈利质量 / 现金流质量 / 结构杠杆 / 风险结论 / 缺口纪律），只在有
  [已选事实] 块时注入；固定研究路径的报告步骤指令同步点名。
- **财务分析图**（图表侧）：`chart_rows`（以底稿为准、**只保留契约期间**）→
  `financial_research_specs`（两期核心指标对比分组柱 / 同比增速 / 盈利与现金流质量）
  → 渲染模板新增 `render_grouped_bar`。研究任务**只出财务图**（检索统计图与"市场规模"
  兜底不再进交付）；顺带修 `topic_terms` 把接口英文字段名当热词。
- **分析覆盖检查**（验收侧，**只提示**）：`check_analysis_completeness` 逐 (必需指标, 年度)
  核对正文数值、派生读数与风险/结论段落；`counted=False` → 缺口进
  `checks.analysis_completeness`，不进 overall（按产品负责人选择：先观察再决定是否收紧）。
- 单测 10 项（比率 4 / 分析覆盖 3 / 图表 3，含**真跑渲染脚本**出 3 张 PNG）；
  我逐张看过图：分组柱带数值标注、图例与数据算出的结论，可读。
- 修了一处自伤缺陷：新增方法把 `@staticmethod` 装饰器顶位，导致 `_wants_visualization`
  失静态、连带三个套件红——回归跑出来的，已修并复跑全绿。
