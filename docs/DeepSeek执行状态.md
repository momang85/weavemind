...
- **新增 P0 实测发现（冷启动竞态）**：新实例首次启动 15/16、编排器静默死掉。faulthandler 抓到栈：`orchestrator_v2:451 → common.py:281 MessagingClient → _connect:307 → redis ping → retry.py:135 call_with_retry`；`common.MessagingClient` 建客户端时未关 redis-py 默认重试（其它模块都已统一为 `Retry(NoBackoff(), 0)`）。Redis 已热后重启即 16/16（对照验证）。建议：MessagingClient 关闭默认重试并明确超时 + 启动器拉服务前等一次成功 PING + 启动校验摘要带上失败原因。细则见 `docs/新手实测报告_20260914.md` §2.5
