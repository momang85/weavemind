...
**当前批次**：实机端到端验证 + 五处真问题修复（浏览器视觉测试驱动），并整理完整测试报告。
证据：`docs/evidence/e2e_test_report_20260921.md`（分钟级运行报告与复测记录）、
...
| 修复 | ①同页两个"当前版本"（复核栏正文版号 vs 导出行谱系标识）→ 导出块双命名空间；②正文图表全部 404（整段编码把 `charts/` 编成 `%2F`）→ 逐段编码；③1038×121366 超高图页面隐形 → 同 chart_qa 阈值占位；④装配不幂等（`****`/重复/回食压缩提示与待核查头/标题成主张，两轮 0→3→6）→ 修复并回归，补修"两个标题"残留；⑤清单重写未重建 zip → `package_stale` 如实标注 |
| 验证 | 修订版 PDF：8 页、6/6 图嵌入在页、0 占位、表格对齐；离线三场景 risks 5/4/7 零伪影；实机正文复跑风险提取 4 条干净；test_delivery_chain 248、guards 42、scenario_checks 17、offline_delivery 32 全绿；面板四行读数 DOM 实测 + 截图 |
| 遗留 | 二次装配取舍（编排层待决策，未擅改）；反思重做稿（分析节 891 字）未进交付；分析节 214 字双成因（节名过滤 + 输出上限耗尽）；新闻稿被归类为"财务附注"；现采纳正文仍带旧渲染垃圾（干净候选 8b23aad3 未采纳）；评审未完成（降级） |
