# DeepSeek 执行状态（2026-09-26 更新 · 新人专项 N0–N4 全部交付）

**当前批次**：新人使用专项 N · **N4 新人路径验收矩阵**（指令
`docs/新人一键启动与首次研究体验_20260925.md` §6）——N0–N3 见 `6715543`、`e81dcf3`、
`e858f0f`、`c1ade62`。九条场景逐条给出状态与证据；**干净环境相关行如实标为未验**。

- **本轮实测通过（本机）**：③ 中文与空格路径（解压到 `…\织光 运行包 验收 2026\解压目录\` 后
  包内解释器导入 launcher/task_intent/dep_check/web_ui 正常、`runtime_identity()` =
  `package:2026.09.26`、渲染 1 页中文 PDF 5,193,040 字节且可提取中文）；⑥ 连续双击复用实例
  （16 个 PID 前后相同、失败不计漏）；⑨ 修订/导出/重启保留账户与配置（前批整链 + 本批重启）。
- **部分通过**：② 包内自带 3.11.9 独立运行时（本机系统 Python 是 3.14）；⑤ 自定义端口单一来源
  与就绪探测（行为用例），真机 Redis 5/端口占用未验；⑦ 演示与真实分开、错误在原页面纠正，
  错误密钥端到端未验；⑧ 公司研究在无 Docker 机器上跑通，含代码任务的提示未跑真机。
- **未验**：① 干净 Windows（无 Python/Node/Docker）整链；④ 真机断网演练。
  需要补的条件与"本轮不做"写在证据文档里（不发布 Release、不上传数据、不为凑通过改口径）。
- **本批付费请求数 0**；未改模型/权限/模板/配置。
- 证据：`docs/evidence/n4_newcomer_acceptance_matrix_20260926.md`。
  专项总览：N0/N1 `n0_n1_single_start_and_readiness_20260926.md`、`n1_startup_controller_20260926.md`；
  N2 `n2_run_package_20260926.md`；N3 `n3_first_run_guide_20260926.md`。
