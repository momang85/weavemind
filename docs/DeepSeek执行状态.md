...
**当前批次**：**S1（一个公司研究包，先固定输入与验收契约）**。S0 已全部收尾；
S1-1（三处 P1）、S1-2（契约 + 最小事实记录）、S1-3（可重算底稿 + 退出关卡）均已落地并推上远端。

**当前提交**：`6077ce8`（S1-3）· 远端 `main` 已同步。

**已验证（按提交分别记账）**

- **CI 四作业全绿**：`9a7b977`（S0 全部改动）与 **`6077ce8`**（S1-1/S1-2/S1-3，run `35214951068`）。
...
- **真实上市公司研究仍未跑过**：S1 的夹具都是**合成数据**，证明的是"契约/事实/底稿/判定"
  接通，不是真实抓取与真实报告质量；美股真实 SEC 抓取仍待验。
- 底稿/缺口表**还没接进任务产物**（报告 + CSV/JSON + manifest 的出口）——那是 S1 收尾。
- 银行投研/对公试点、港股/美股真实覆盖：按新指令后置，前置门槛不降。
- Linux 侧逐页渲染仍只在 CI 单测层面确认（未在 Linux 上跑 `scripts/pdf_visual_check.py`）。

**下一步**：S1 收尾（把底稿/缺口表接进任务产物与导出）→ S2（首页默认"研究一家公司"、
结果页 fact/来源定位、复核改版重验、中文与时间显示）→ S3（非开发者试用任务包与最小指标）。
