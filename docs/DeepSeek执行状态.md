...
**当前批次**：F2′（来源身份、证据适用性与成稿验收闭环）——代码与单测完成；实机为 F2 的两次
（已按原停止条件收尾，不追加第三轮）。证据：`docs/evidence/report_analysis_F2_20260919.md`
（§6 F2′ 反例前后、§7 真实年报 PDF 定位、§8 两次实机回填）。

| 层 | 状态 |
|---|---|
| 功能实现 | F2′ 三项均已落地：①引用重编号映射 + 来源按域名分级 + 证据契约校验（主体/期间/资料截止）；②真实年报 PDF 文本提取与**页码定位**（`annual_report_pdf.py`，复用 pypdf 与既有公网校验通道）；③成稿顺序（先装配候选稿再对同版验收）、分析保留、来源修复只动真声明 |
| 单测 | 新增 20 项（契约适用性 5 / PDF 与页码 4 / 实机失败回放 1 / 引用映射与来源分级 4 / 分析保留与成稿顺序 5 / 来源修复根不变量 1）；相关 11 个套件 691 项通过 |
| 实机 | **F2 两次**：`ui-ceb4b710d1`（投研，18m55s，SUCCESS_WITH_ISSUES，draft，9 次调用）四节结构成立、缺口如实；`ui-636b845c22`（银行对公，23m02s，达上限停止，3 次验收 fail 进入重做）。四类叙事证据实机为空（搜索降级 + WAF/付费墙 + 当时无 PDF 通道）。**F2′ 尚未端到端实机**（按指令不加第三轮） |
| 人工验收 | **未做**（架构指令里属 F3：研究员五项评分 ≥8/10） |
