...
**当前批次**：F2（让简报回答金融研究问题）——代码与单测完成；**两次实机已跑，证据侧未通过**。
证据：`docs/evidence/report_analysis_F2_20260919.md`（§5 实机读数）。

| 层 | 状态 |
|---|---|
| 功能实现 | F2-1 定向取证（`narrative_evidence.py` + 研究路径两个抓取步骤）、F2-2 简报四节（业务背景/变化解释/风险与核查/附录定位）、F2-3 两种阅读视角 + 比率适用条件——均已落地 |
| 单测 | 新增 `test_narrative_evidence.py` 15 项 + 扩展 20 余项；CI 列出的 46 个后端测试文件本地逐个跑通，前端 45 项行为测试通过；CI（`ae79ae1`）四作业全绿 |
| 真实报告 | **实机两次**：`ui-ceb4b710d1`（投研，18m55s，SUCCESS_WITH_ISSUES，交付 **draft**）四节结构成立、缺口如实；`ui-636b845c22`（银行对公，23m 达上限被停止）3 次验收 fail 进入重做循环。**四类叙事证据实机为空**：DuckDuckGo 不可达（代理 7897 挂了）→ 搜索走 Bing 兜底返回意见稿/无关页；抓到的页面是 WAF 载荷、"会员可见"或无关页；A 股年报是 PDF 且无解析 |
...
**F2 实机暴露、留给 F3 的三件事**（读数见证据 §5）
1. **搜索降级无兜底**：DuckDuckGo 不可达时只剩 Bing 兜底，返回意见稿/无关页（同公司同日 06:03 的候选
   里有发行人年报 PDF，19:07/19:32 只剩「贵州茅台的盈利会下降吗？」与 Statista）。
2. **年报多为 PDF 且无解析**：A 股年报/研报候选都是 `.pdf`，`WebFetchWorker` 拿不到正文（cninfo 桩未实现）。
   即便抓到了，雪球页也是反爬 WAF 载荷、前瞻眼页字段"会员可见"。
3. **验收器在模型正文上跑**：要求模型重做装配器本就会补齐的东西（`ui-636b845c22` 第一次 fail 就是
   "缺少免责声明"，而简报装配器必定写 `### 免责声明`），触发"按缺口强制重做"循环直到触达上限。

**下一步**（架构指令顺序）：F3 图表分面、修订前端入口、离线三场景与人工评分（含上面三件事）→ D 真实试用。
