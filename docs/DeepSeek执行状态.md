# DeepSeek 执行状态（2026-09-26 更新 · 中国平安式无 Docker 任务新人视角复跑）

**当前批次**：用户授权的一次真实付费研究任务（新人视角、无 Docker、真机端点），
用于回答"之前的中国平安问题解决了吗"。交付物：运行包
`dist/weavemind-2026.09.26.7-win-x64.zip`。

- **成因已消失**：本次计划 6 步（`web_search → web_fetch → web_fetch → content_summary →
  report_generator → package`）**不含 `code_execution`**——中国平安那次"计划含代码步骤 → 无 Docker
  被拒 → 修复轮空转十几分钟"的链路没有再现；任务 12 分钟跑到终态，外部检索失败后**自动重规划**
  改走结构化财务数据（`Replan (structured-source fallback)`），产出 7,200 字报告。
- **结果如实**：任务终态 FAILED，报告自带四行免责声明（未验收草稿／研究草稿待补原始披露
  located=0、必答问题 3/3 未完成／研究交付硬门槛未通过／评审未完成降级）；机器验收
  `overall: pass, version_bound: true`（数字与格式规则通过，与研究就绪分开写）。
- **复跑又暴露并修掉三个真问题**：① **两个实例共用一台 Redis → 任务被另一个实例抢去执行**
  （`orchestrator:main` 是 pub/sub，广播给所有编排器；已改为"默认端口上的 Redis 不是本实例启动的
  就不复用，自起空闲端口"，实测开发实例 6379 / 运行包实例 6380 互不干扰；显式配置同一台 Redis
  时仍会串台，未修，如实记录）；② **模板里写死的角色/规划/备用端点让首次任务评审直接降级**
  （`deepseek-chat` 该端点不提供、planner/backup 指向别家厂商 + 占位密钥；已把模板三项留空并加
  占位符守卫，验证 plan/exec/judge 全部解析到主模型）；③ **配置缺失回退到硬编码 stub**，
  新手实例丢了模板默认值（评审被关、超时 90 秒、预算 0=不限；已改为回退随包模板 + 占位符不算
  已配置）。
- **本批付费请求**：1 次研究任务（用户授权；费用未知，端点未提供价格表）+ 少量探测请求；
  未改模型/权限/`templates.json`；未发布 Release、未上传数据。
- **未验**：三处修复后**没有再跑第二次付费任务**（授权是"一次"）——3.1 为实机验证，3.2/3.3 为
  代码级验证（角色解析结果 + 模板断言 + 单测）；干净机器/物理断网/真实 Redis 5 仍未验。
- 证据：`docs/evidence/pingan_style_live_run_20260926.md`（含报告原文、40 条任务日志、
  验收 JSON、检索失败记录）。

## 归档批次（N4 场景复验、N0–N4、R1–R4 与更早）

### N4 场景实机复验（2026-09-26 白天）：干净环境近似、断网、端口冲突、错误密钥、代码隔离提示
