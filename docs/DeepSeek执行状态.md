...
**当前批次**：**S2 进行中**（研究入口与结果页）。S1 已完成（契约 → 事实 → 可重算底稿 →
产物出口，含真实数据整链验证）。
...
**S2 剩余**：结果页顺序再按"先结论/目标达成"微调；导出（ZIP/清单）里带上底稿 CSV/JSON；
时间本地化、阶段/等待对象/取消、草稿与降级标记的逐项复核；**需要登录态的渲染检查**
（桌面/窄屏、运行/成功/缺口/取消、SSE 重连、切任务不串状态）——按纪律不索取/复制 cookie，
登录由用户完成，离线页面检查不替代它。

**下一步**：S2 剩余项 → S3（非开发者试用任务包与最小指标）。
