...
**当前批次**：**S0（PDF 与交付门禁收尾）**。S0-2 逐页视觉已做（视觉门禁两轮，修复后 3/3 通过）；
S0-3 部署烟测假绿判据已修并加负向用例（本机 EXIT=0）；S0-4/S0-5 命名与文档已同步。

**当前提交**：`e7697a7`（字体回退）→ `b8ab5cb`（视觉门禁发现的两处排版缺陷 + 视觉证据）
→ `ae930b2`（烟测判据 + 负向用例）→ `cbde174`（替身规划识别收窄）→ 本轮文档提交。

**已验证（按提交分别记账，不互相套用）**

- **CI 四作业全绿**：`ec4f942`（首绿）与 **`9a7b977`**（当前批次，run `35213467639`）。
  后者覆盖了本轮全部改动：Linux 侧 PDF 字形修复（cmap 合并 + 缺字形回退内置 Helvetica）、
  逐页视觉发现的两处排版缺陷、部署烟测的**内容判据 + 负向用例**、S1-1 跨层保真测试。
- `b280cea` 的 CI 曾只报一处**空白断言**差异，已在 `e7697a7` 修掉并随后全绿；
  该提交本身不代表当前状态。
- 顺带确认：**Linux 上的字体问题确实只在 Linux 复现**，Windows 本地通过并不代表它好了
  （修复后 CI 绿才算数）。
- 本机全量：`test_p0` 399、`test_orchestrator_v2` 68、`test_delivery_chain` 184、
  `test_offline_delivery` 8、`test_root_budget` 37、`test_cancel_semantics` 46 等全绿。
- PDF 逐页视觉：**Windows 上**两轮判定（8 页→6 通过/2 缺陷；修复后 3/3 通过），
  证据 `docs/evidence/pdf_visual_20260917.md`（含渲染命令、页码、PDF/PNG sha256）。
- 部署烟测：`scripts/e2e_clean_check.py` 本机 EXIT=0（正向 SUCCESS + 夹具内容判据通过；
  负向 FAILED + 判据拒绝），证据 `docs/evidence/e2e_clean_last.json`。

**未验证**

- Linux 侧 PDF 字形修复（cmap 合并 + 缺字形回退）**只能由 CI 或 Linux 上跑
  `scripts/pdf_visual_check.py` 确认**；Windows 通过不能替代。
- 真实模型质量、真实 SEC/行情抓取、金融数字正确性：**本轮一次都没测**
  （部署烟测用固定替身，不证明这些）。
- 代理恢复前无法核对 `b280cea`…`cbde174` 的云端 CI。

**下一步**：S1 —— 先修已定位的 `structured_pipeline` P1（合并 clean 丢主体/metadata、
金额硬写"亿元"、acceptance 优先 clean 命中绕过主体校验），再做"一公司两年度 × 三核心指标 ×
来源位置 × 可重算底稿"的闭环。
