...
- **未修挂账**：运行中刷新页面不再跟踪任务（后端仍有该任务日志，页面无）；
  文档级数字-主体绑定缺口（既有）。
- **已修（本轮后续）**：`identity_id()` 漂移——`bind_acceptance` 改写 `rules_fingerprint`
  后重算身份与入库键不再相等（实测 `7faeb4d9…` vs `2033fae7…`）。现在**入库时算出的
  身份是权威**，另加 `identity_drifted()` 审计信号；真实任务上验证页面/导出/清单的版本
  标识都回到 `7faeb4d9…`（与版本库的键一致）。
