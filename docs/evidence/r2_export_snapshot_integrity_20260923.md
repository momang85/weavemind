...
## 7. R2 退出：正常页面整链 + 包内可复算（实机）

服务以 R1/R2 代码重启（`launcher.py stop` → `deps --fix` → `start`，16/16 存活），随后在
页面完成整链（任务 `ui-706c5ef4a5`）：

1. **查看差异**：详情面板显示当前版 `6f2c004d1b52`、机器验收 pass、包陈旧提示
   （"重装配未重建包，包可能不含最新修订"）；
2. **修改 + 重验 + 采纳**：修订框粘贴候选（11,209 字符）→ 提交后页面显示
   `新版本 aac5d6415bff（父 6f2c004d1b52）；验收 pass，交付已绑定该版本`；
3. **重包**：点"按当前版本重新导出" → `deliverables_20260923_230558_3d22ce.zip`（23:05），
   页面陈旧提示消失；
4. **包内可复算（离线核对）**：
   - 17 个成员（16 冻结成员 + 清单），逐成员 hash 复算**全部一致**；
   - `report_version_id=f5f71249324a40e8`、`research_body_sha256=aac5d6415bffa24a`、
     `binding_verified=True`、`logic_fingerprint=96b1881262aa7b6b`、
     `delivered_md_sha256=511853d07dd0320f`、`pdf_sha256=d95c112a4d0992b2`；
   - 包内 MD 与任务库送达正文**逐字节相同**（页面下载同源）；
   - 引用证据带 `chunk_offsets_by_doc` / `chunk_gaps_by_doc`；跨缺口记录
     `risk` 标 `after_gap=[6,7,8,9]`，定位写明"上接缺口 6–9，为分段摘录、非连续原文"；
     逐条 `chunk_char_start/end` 按**该记录自己的文档**换算（样例：chunk 2 段内
     2744-3025 / 3025-3996 / 3996-5195，chunk 3 段内 2475-2854）；
   - `drift` 只报 `evidence/citation_evidence.json: changed_on_disk`（工作区那份是离线重入时写的，
     包内那份来自本次快照）；
5. **包内 PDF**：12 页，逐字符检查**无越界**（各页 `min_bottom=24.3`、`max_top≤788.8<841.9`），
   交付 MD 的每个标题与关键句（结论尾句、边界、比率适用条件、参考来源、免责声明）全部可找到。

采纳后的正文语义（与问题区/风险区/状态同源）：收入=`分解覆盖（部分）`、
利润=`仅读数/观察`、现金=未取得；头部状态为"必答问题未完成（3/3，其中部分覆盖 1 项）"。
