...
_NEUTRAL_SECTION_WORDS = (
    ...
    "方法", "总览", "明细", "注", "目录", "序言",
    # 装配器（report_brief）与常见研报小节名：它们是**中性小节**，不是别家公司。
    # 少了这些词，"## 关键发现"这类 4 字 CJK 标题会落到"短专名 → 切断继承"那一档，
    # 把本公司的数字判成"未绑定主体"（F1 实机测试暴露）。
    "关键发现", "变化解释", "业务背景", "经营分析", "核查", "待核查", "资料范围",
    "版本", "验收", "质量", "结构", "杠杆", "盈利", "摘要与结论", "重点",
    # D3：研究问题小节同样是**装配器小节名**（含底稿读数的观察），不是"别家公司"
    "研究问题", "下一步", "核查动作", "推断边界",
)
_COMPANY_HINT_WORDS = (
